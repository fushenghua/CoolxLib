...
public class Table {
    ...
    private Table(CooDb db, Class<?> entityType) {
        this.db = db;
    ...
    public static synchronized Table get(CooDb db, Class<?> entityType) {
    ...
    public static synchronized void remove(CooDb db, Class<?> entityType) {
        String tableKey = db.getDaoConfig().getDbName() + "#" + entityType.getName();
        tableMap.remove(tableKey);
    }

    public static synchronized void remove(CooDb db, String tableName) {
        if (tableMap.size() > 0) {
            String key = null;
